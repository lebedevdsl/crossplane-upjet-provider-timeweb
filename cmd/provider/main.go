...
import (
	...
	apisCluster "github.com/lebedevdsl/crossplane-upjet-provider-timeweb/apis/cluster"
	apisNamespaced "github.com/lebedevdsl/crossplane-upjet-provider-timeweb/apis/namespaced"
	"github.com/lebedevdsl/crossplane-upjet-provider-timeweb/config"
	"github.com/lebedevdsl/crossplane-upjet-provider-timeweb/internal/clients"
	controllerCluster "github.com/lebedevdsl/crossplane-upjet-provider-timeweb/internal/controller/cluster"
	controllerNamespaced "github.com/lebedevdsl/crossplane-upjet-provider-timeweb/internal/controller/namespaced"
	"github.com/lebedevdsl/crossplane-upjet-provider-timeweb/internal/features"
	"github.com/lebedevdsl/crossplane-upjet-provider-timeweb/internal/version"
)
...
func main() {
	var (
		app                     = kingpin.New(filepath.Base(os.Args[0]), "Terraform based Crossplane provider for Timeweb").DefaultEnvars()
	...
	log := logging.NewLogrLogger(zl.WithName("provider-timeweb"))
	if *debug {
	...
	mgr, err := ctrl.NewManager(cfg, ctrl.Options{
		...
		LeaderElectionID: "crossplane-leader-election-provider-timeweb",
		Cache: cache.Options{
	...
	kingpin.FatalIfError(apisCluster.AddToScheme(mgr.GetScheme()), "Cannot add cluster-scoped Timeweb APIs to scheme")
	kingpin.FatalIfError(apisNamespaced.AddToScheme(mgr.GetScheme()), "Cannot add namespaced Timeweb APIs to scheme")
	...
	if canSafeStart {
		...
		kingpin.FatalIfError(controllerCluster.SetupGated(mgr, clusterOpts), "Cannot setup cluster-scoped Timeweb controllers")
		kingpin.FatalIfError(controllerNamespaced.SetupGated(mgr, namespacedOpts), "Cannot setup namespaced Timeweb controllers")
	} else {
		log.Info("Provider has missing RBAC permissions for watching CRDs, controller SafeStart capability will be disabled")
		kingpin.FatalIfError(controllerCluster.Setup(mgr, clusterOpts), "Cannot setup cluster-scoped Timeweb controllers")
		kingpin.FatalIfError(controllerNamespaced.Setup(mgr, namespacedOpts), "Cannot setup namespaced Timeweb controllers")
	}
