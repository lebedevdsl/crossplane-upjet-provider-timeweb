...
PROJECT_NAME ?= provider-timeweb
PROJECT_REPO ?= github.com/lebedevdsl/crossplane-upjet-provider-timeweb
...
export TERRAFORM_PROVIDER_SOURCE ?= timeweb-cloud/timeweb-cloud
export TERRAFORM_PROVIDER_REPO ?= https://github.com/timeweb-cloud/terraform-provider-timeweb-cloud
export TERRAFORM_PROVIDER_VERSION ?= 1.6.16
export TERRAFORM_PROVIDER_DOWNLOAD_NAME ?= terraform-provider-timeweb-cloud
export TERRAFORM_PROVIDER_DOWNLOAD_URL_PREFIX ?= https://github.com/timeweb-cloud/terraform-provider-timeweb-cloud/releases/download/v$(TERRAFORM_PROVIDER_VERSION)
# Binary name AFTER the Dockerfile's rename step. The upstream zip contains
# terraform-provider-timeweb-cloud_v<VER>; we rename it to terraform-provider-twc
# so the filesystem mirror path (.../timeweb-cloud/twc/...) and TF's expected
# binary name agree. See cluster/images/provider-timeweb/Dockerfile.
export TERRAFORM_NATIVE_PROVIDER_BINARY ?= terraform-provider-twc
...
xpkg.build.provider-timeweb: do.build.images
...
# pull-docs is a no-clone override: the TF provider source repo is vendored as
# a git submodule at terraform-provider-timeweb-cloud/. We symlink it into the
# .work/ path that upjet's docs reader expects.
pull-docs:
	@if [ ! -d "$(CURDIR)/terraform-provider-timeweb-cloud/docs/resources" ]; then \
		git submodule update --init terraform-provider-timeweb-cloud; \
	fi
	@mkdir -p "$(dir $(WORK_DIR)/$(TERRAFORM_PROVIDER_SOURCE))"
	@rm -rf "$(WORK_DIR)/$(TERRAFORM_PROVIDER_SOURCE)"
	@ln -s "$(CURDIR)/terraform-provider-timeweb-cloud" "$(WORK_DIR)/$(TERRAFORM_PROVIDER_SOURCE)"
